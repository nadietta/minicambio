...
if ($result =  mysqli_query($conn,$query)) {
   ...
   while ($row = $result->fetch_row()) {
        ...
        $risultato[$i]['tasso_medio_giorno']=number_format($row[11], 4, ',', '.');
        $risultato[$i]['cod_op']=$row[7];
        $risultato[$i]['tipo_op']=$row[8];
        $risultato[$i]['totale_entrata']=number_format($row[9], 2, ',', '.');
        $risultato[$i]['totale_uscita']=number_format($row[10], 2, ',', '.');
        $risultato[$i]['fk_entrata']=$row[12];
        $risultato[$i]['fk_uscita']=$row[13];
        $risultato[$i]['mese']=$row[14];
        $risultato[$i]['totale_entrata_mese']=number_format($row[15], 2, ',', '.');
        $risultato[$i]['totale_uscita_mese']=number_format($row[16], 2, ',', '.');
        $risultato[$i]['tasso_medio_mese']=number_format($row[17], 4, ',', '.');
